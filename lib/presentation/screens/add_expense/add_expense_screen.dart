      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          actions: [
            ...
            TextButton(
              onPressed: isGlobalLoading ? null : _saveExpense,
              child: Text(isGlobalLoading ? 'Saving...' : 'Save'),
            ),
          ],
        ),
      body: Form(
  ...
  Future<void> _saveExpense() async {
    ...
    try {
      if (isOnline) {
        ...
        await ref.executeWithTrackedLoading('create_expense', () async {
        ...
        await syncNotifier.queueOperation(SyncOperation(
          id: 'create_expense_${DateTime.now().millisecondsSinceEpoch}',
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Expense saved offline - will sync when online 📱'),
